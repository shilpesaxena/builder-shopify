...
export default function TemplatesDropdown({data} : any) {
  const templateData = data?.filter((item: { name: string; }) => item.name==="Template")
                  ...
                  <div className="container">
                    <div className="flex text-sm border-t border-slate-200 dark:border-slate-700 py-14">
                      <div className="flex-1 grid grid-cols-4 gap-6 xl:gap-8 pr-6 xl:pr-8">
                        {templateData?.[0]?.children?.map((item : any, index : number) => (
                          <div key={index}>
